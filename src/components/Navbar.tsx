...
const Navbar: React.FC<NavbarProps> = ({ onLogin, onRegister }) => {
  ...
  return (
    <header className="p-2 md:p-4">
      <nav className="container mx-auto max-w-7xl flex justify-between items-center p-2 md:p-4 rounded-xl glass-effect-modal">
        ...
        <div className="md:hidden flex items-center">
          <button 
            ...
            className="w-9 h-9 flex items-center justify-center bg-white/90 border border-gray-200 shadow-md rounded-full p-0.5 focus:outline-none focus:ring-2 focus:ring-sky-400 transition-all"
      ...
      {/* Mobil Menü - Modern, küçük, kontrastlı */}
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex items-start justify-end bg-black/40 backdrop-blur-sm">
          <div className="w-[85vw] max-w-xs mt-4 mr-3 bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 animate-in slide-in-from-right-2 relative">
            ...
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-base font-bold text-sky-600">Baykuş</span>
              </div>
            ...
            <div className="space-y-1 mt-1">
              <a href="#catalogs" className="flex items-center gap-2 text-gray-700 hover:text-sky-600 px-3 py-2 rounded-lg hover:bg-sky-50 transition-colors text-sm">
              ...
              <a href="#" className="flex items-center gap-2 text-gray-700 hover:text-sky-600 px-3 py-2 rounded-lg hover:bg-sky-50 transition-colors text-sm">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17v-6a2 2 0 012-2h2a2 2 0 012 2v6" /></svg>
                Sınavlar
              </a>
              <a href="#" className="flex items-center gap-2 text-gray-700 hover:text-sky-600 px-3 py-2 rounded-lg hover:bg-sky-50 transition-colors text-sm">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16h6" /></svg>
                Hakkımızda
              </a>
            </div>
            <div className="border-t border-gray-200 mt-2 pt-2 space-y-1">
              <button 
                className="w-full text-left text-gray-700 hover:text-sky-600 px-3 py-2 rounded-lg hover:bg-sky-50 transition-colors font-semibold text-sm" 
                onClick={() => {
                  onLogin();
              ...
              <button 
                className="w-full bg-sky-500 text-white font-bold py-2 rounded-lg hover:bg-sky-600 transition-colors shadow-lg text-sm" 
                onClick={() => {
                  onRegister();
                  setIsMobileMenuOpen(false);
                }}
